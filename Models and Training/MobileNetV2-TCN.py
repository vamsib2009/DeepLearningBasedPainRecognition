...
class CNNFeatureExtractor(nn.Module):
    def __init__(self, in_channels=6, pretrained=False):
        ...
        # Modify the first conv layer to accept in_channels (default is 3)
        old_first_conv = mobilenet.features[0][0]
        mobilenet.features[0][0] = nn.Conv2d(in_channels, old_first_conv.out_channels,
                                             kernel_size=old_first_conv.kernel_size,
                                             stride=old_first_conv.stride,
                                             padding=old_first_conv.padding,
                                             bias=False)

        self.feature_extractor = mobilenet.features  # exclude classifier
        self.pool = nn.AdaptiveAvgPool2d((1, 1))     # output: (B*T, 1280, 1, 1)
        self.dropout = nn.Dropout(0.25)

    def forward(self, x):  # (B*T, C, H, W)
        features = self.feature_extractor(x)         # (B*T, 1280, H', W')
        features = self.pool(features)               # (B*T, 1280, 1, 1)
        features = features.view(features.size(0), -1)  # (B*T, 1280)
...
class CNN_TCN_Model(nn.Module):
    def __init__(self, sequence_length, num_classes):
        ...
        self.tcn = TCN(input_size=1280, num_channels=[512, 256, 128], kernel_size=3, dropout=0.3)
    ...
    def forward(self, sequence_mhi, sequence_ofi):
        ...
        cnn_features = self.cnn(combined)  # (B*T, 1280)
        cnn_features = cnn_features.view(B, T, -1)  # (B, T, 1280)
        cnn_features = cnn_features.permute(0, 2, 1)  # (B, 1280, T)
...
# ---------------------- LR Scheduler ----------------------
def adjust_learning_rate(optimizer, epoch, initial_lr, decay_factor=0.1, decay_every=9):
...
for epoch in range(num_epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        best_epoch = epoch + 1
        patience_counter = 0
        torch.save(model.state_dict(), f'results/best_model_{test_subject_id}.pt')
        #torch.save(quantized_model.state_dict(), f'quantized_results/quantized_model_{test_subject_id}.pt')
    else:
        patience_counter += 1
...
model.load_state_dict(torch.load(f'results/best_model_{test_subject_id}.pt'))
model.eval()
